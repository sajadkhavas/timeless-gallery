      ...
      <section className="border-b border-hairline px-6 pb-20 pt-12">
        <div className="mx-auto max-w-7xl">
          ...
          <div className="grid gap-12 md:grid-cols-[1.1fr_1fr] md:gap-16">
            ...
            <div>
              <p className="text-[10px] uppercase tracking-[0.35em] text-primary">{p.brand} ـ {p.collectionLabel}</p>
              ...
              <div className="mt-6 flex items-center gap-3 text-xs">
                ...
                <span className="text-foreground/50">({p.reviews.toLocaleString("fa-IR")} نظر)</span>
              ...
              <div className="mt-8 flex items-baseline gap-3">
                {p.oldPrice && (
                  <span className="text-sm text-foreground/40 line-through">{formatToman(p.oldPrice)}</span>
                )}
                <span className="text-3xl font-medium text-primary">{formatToman(p.price)}</span>
                <span className="text-[11px] uppercase tracking-[0.3em] text-foreground/50">تومان</span>
              </div>
              ...
              <div className="mt-10 flex flex-wrap gap-3">
                <button className="bg-primary px-6 py-3 text-sm font-medium text-primary-foreground hover:bg-primary/90" disabled={!p.inStock}>
                  افزودن به سبد خرید
                </button>
                <button className="border border-hairline px-6 py-3 text-sm font-medium hover:border-primary">
                  ذخیره در علاقه‌مندی
                </button>
                <Link to="/contact" className="border border-hairline px-6 py-3 text-sm font-medium hover:border-primary">
                  مشاوره‌ی اختصاصی
                </Link>
              </div>
            ...
            <div className="py-10 text-sm leading-relaxed text-foreground/75">
              {tab === "specs" && (
                <dl className="grid gap-x-12 gap-y-4 sm:grid-cols-2">
                  {[
                    ...
                    <div key={k} className="flex justify-between gap-4 border-b border-hairline py-3">
              ...
              {tab === "story" && (
                <p className="max-w-3xl text-pretty leading-loose">
                  {p.story} این قطعه با وسواس در آتلیه‌ی نوا گزینش شده و پیش از تحویل،
                  بازرسیِ کامل کالیبراسیون، مقاومت آب و اصالت قطعات را پشت سر گذاشته است.
                </p>
              )}
              {tab === "guarantee" && (
          ...
          <div className="mt-12">
            ...
            <div className="mt-6 grid gap-6 md:grid-cols-2">
              {[
                { n: "آرمان ر.", t: "ساخت بی‌نقص و بسته‌بندی فوق‌العاده. حس واقعی یک قطعه‌ی لوکس." },
                ...
                <div key={r.n} className="hairline p-6">
                  ...
                  <p className="mt-4 text-[11px] uppercase tracking-[0.25em] text-foreground/45">{r.n}</p>
                </div>
              ))}
            </div>
          </div>
