...
export const Route = createFileRoute("/collections")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "مرور کالکشن‌های ساعت‌های لوکس نوا؛ کلاسیک، اسپرت لوکس و نسخه‌ی میراثی.",
      },
      ...
      <section className="px-6 py-20">
        <div className="mx-auto grid max-w-7xl gap-16">
          {COLLECTIONS.map((c, i) => (
            <article
              key={c.slug}
              className={`grid items-center gap-10 md:grid-cols-2 md:gap-16 ${i % 2 ? "md:[direction:ltr]" : ""}`}
            >
              <Link
                to="/shop"
                search={{ collection: c.slug }}
                className="group block md:[direction:rtl]"
              >
              ...
              <div className="md:[direction:rtl]">
                ...
                <p className="mt-6 max-w-md text-pretty text-base leading-loose text-foreground/70">
                  {c.desc}
                </p>
                ...
                <Link
                  to="/shop"
                  search={{ collection: c.slug }}
                  className="mt-10 inline-flex items-center gap-3 border-b border-primary/40 pb-2 text-sm font-medium hover:border-primary"
                >
                  ورود به کالکشن
